...
public class Visit_Report_Controller {
	...
	@PostMapping("/")
	public ResponseEntity<Visit_Report> SaveVisitReport(@RequestBody Visit_Report_DTO visit_Report_DTO){
	...
	@PutMapping("/{id}")
	public ResponseEntity<Visit_Report> UpdateAllVisitReportById(@PathVariable("id") int id, @RequestBody Visit_Report_DTO visit_Report_DTO){
		Visit_Report visit_Report=visit_Report_Service.GetAllVisit_ReportByid(id);
		if(visit_Report==null) {
			return new ResponseEntity<>(HttpStatus.NOT_FOUND);
		}visit_Report.setCreated_at(visit_Report_DTO.getCreated_at());
