...
public class Diagnostics_Controller {
	...
	@PostMapping("/")
	public ResponseEntity<Diagnostics> SaveDiagnostics(@RequestBody Diagnostics_DTO diagnostics_DTO){
		Diagnostics diagnostics = new Diagnostics();
		
		diagnostics.setName(diagnostics_DTO.getName());
		diagnostics.setCreate_at(diagnostics_DTO.getCreate_at());
		
		Docter docter=docter_Service.getBYID(diagnostics_DTO.getDocterid());
		diagnostics.setDocterid(docter);
	...
	@GetMapping("/{id}")
	public ResponseEntity<Diagnostics> getDiagnosticsById(@PathVariable("id") int id,@RequestBody Diagnostics_DTO diagnostics_DTO){
	...
	@PutMapping("/{id}")
	public ResponseEntity<Diagnostics> UpaddteDiagnosticsById(@PathVariable("id") int id, @RequestBody Diagnostics_DTO diagnostics_DTO){
		Diagnostics diagnostics = diagnostics_Service.getByDiagnosticsID(id);
		if(diagnostics==null) {
			return new ResponseEntity<>(HttpStatus.NOT_FOUND);
		}
	...
	@DeleteMapping("{id}")
	public ResponseEntity<Void> DeleteDiagnosticsById(@PathVariable("id") int id , @RequestBody Diagnostics_DTO diagnostics_DTO){
		Diagnostics diagnostics = diagnostics_Service.getByDiagnosticsID(id);
		if(diagnostics==null) {
			return new ResponseEntity<>(HttpStatus.NOT_FOUND);
