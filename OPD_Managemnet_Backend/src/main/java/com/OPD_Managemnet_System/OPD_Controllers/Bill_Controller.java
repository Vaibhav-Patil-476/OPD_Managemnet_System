...
public class Bill_Controller {
	...
	@PostMapping("/")
	public ResponseEntity<Bill> SaveBill(@RequestBody Bill_DTO bill_DTO){
		Bill bill = new Bill();
		...
		bill.setToatl_amount(bill_DTO.getToatl_amount());
	...
	@GetMapping("/{id}")
	public ResponseEntity<Bill> GetAllBillById(@PathVariable("id")int id,@RequestBody Bill_DTO bill_DTO) {
	...
	@PutMapping("{id}")
	public ResponseEntity<Bill> UpdateAllBillById(@PathVariable("id")int id,@RequestBody Bill_DTO bill_DTO){
		Bill bill=bill_Service.GetBillById(id);
		if(bill==null) {
			return new ResponseEntity<>(HttpStatus.NOT_FOUND);
	}
