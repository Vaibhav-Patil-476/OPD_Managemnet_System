...
public class Referral_Controller {
	...
	@PostMapping("/")
	public ResponseEntity<Referral> SaveRefarral(@RequestBody Referral_DTO referral_DTO){
		...
		Docter docter=docter_Service.getBYID(referral_DTO.getDocterid());
		referral.setDocterid(docter);
	...
	@PutMapping("/{id}")
	public ResponseEntity<Referral> UpdateReferralById(@PathVariable("id") int id, @ RequestBody Referral_DTO referral_DTO){
		Referral referral = referral_Service.GetReferralById(id);
		if(referral==null) {
			return new ResponseEntity<>(HttpStatus.NOT_FOUND);
		}
