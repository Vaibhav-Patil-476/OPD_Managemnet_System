...
public class Medicine_Controller {
	...
	@PostMapping("/")
	private ResponseEntity<Medicine> SaveMedicine(@RequestBody Medicine_DTO medicine_DTO){
		Medicine medicine = new Medicine();
		
		medicine.setName(medicine_DTO.getName());
		medicine.setType(medicine_DTO.getType());
		
		Docter docter=docter_Service.getBYID(medicine_DTO.getDocterid());
		medicine.setDocterid(docter);
	...
	@GetMapping("/{id}")
	private ResponseEntity<Medicine> GetMedicineById(@PathVariable("id") int id, @RequestBody Medicine_DTO medicine_DTO){
	...
	@PutMapping("/{id}")
	private ResponseEntity<Medicine> UpdateMedicineById(@PathVariable("id") int id, @RequestBody Medicine_DTO medicine_DTO){
	Medicine medicine = medicine_Service.GetMedicineById(id);
	if(medicine==null) {
		return new ResponseEntity<>(HttpStatus.NOT_FOUND);
	}
